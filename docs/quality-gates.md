...
- Advanced routing remains an unqualified application capability. The pinned Chicago graph has passed initial walk, CTA bus and rail, wheelchair, outside-graph, disconnected-client and eight-request concurrency checks; signed API activation, the application-level real journey and the post-activation observation window still must pass. Do not infer routing completion from service-profile tests or router-only evidence.
